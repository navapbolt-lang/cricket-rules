...
async def chat(
    ...
    if not chat_service:
        ...
        raise HTTPException(
            status_code=503,
            detail="Chat service unavailable. Vector database not connected.",
        )
...
async def chat_stream(
    ...
    async def stream_generator():
        ...
        yield {
            "event": "status",
            "data": StreamEvent(
                type="status", message="Analyzing query and searching rulebooks..."
            ).model_dump_json(),
        }
        ...
        for i, word in enumerate(words):
            yield {
                "event": "token",
                "data": StreamEvent(
                    type="token", text=(word + " ") if i < len(words) - 1 else word
                ).model_dump_json(),
            }
        ...
        yield {
            "event": "done",
            "data": StreamEvent(type="done", response=response).model_dump_json(),
        }
...
async def suggestions(
    ...
    logger.info(
        f"Retrieving suggestions for partner: {partner_id}, format: {format}, context: {context}"
    )
    ...
    items = [
        ...
        SuggestionItem(
            question="How does DRS (Decision Review System) work?", category="DRS"
        ),
        SuggestionItem(
            question="When is a delivery called a Wide ball?", category="Delivery"
        ),
        SuggestionItem(
            question="What happens when a ball hits the fielding team's helmet on the ground?",
            category="Dead Ball",
        ),
    ]
    ...
    if format:
        ...
        if fmt == "test":
            items.append(
                SuggestionItem(
                    question="How many DRS reviews are allowed per innings in Test matches?",
                    category="Test Match",
                )
            )
            ...
            items.append(
                SuggestionItem(
                    question="What are the powerplay restrictions in limited overs matches?",
                    category="Powerplay",
                )
            )
