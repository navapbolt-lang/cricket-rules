...
async def get_partner_id(api_key: str = Security(api_key_header)) -> str:
    ...
    if api_key in ["test-key", settings.admin_api_key] or api_key.startswith(
        "partner_"
    ):
...
async def check_rate_limit(
    request: Request, partner_id: str = Depends(get_partner_id)
) -> str:
    ...
    try:
        ...
        if (
            ...
            and int(current_usage) >= settings.default_monthly_quota
        ):
            raise HTTPException(
                status_code=429, detail="Rate limit exceeded. Monthly quota reached."
            )
...
async def get_chat_service(request: Request) -> ChatService:
    """Retrieve ChatService instance from app state, initializing if needed."""
    chat_service = request.app.state.chat_service
    if chat_service is None:
        # Lazy initialization on first request
        request.app.state.init_rag()
        chat_service = request.app.state.chat_service
    return chat_service
